...
def call_once(prompt, retries=2):
    body = json.dumps({
...
def main():
    ...
    base_summary = {
        ...
        "cost_guard": "Gemini 3.6 Flash Free Tier + public standard GitHub-hosted runner; no billing setup invoked.",
    }

    if not API_KEY:
        write_result({
            "summary": {**base_summary, "status": "SECRET_MISSING", "passes": 0, "pass_rate": 0.0},
            "results": [],
            "diagnostic": "GEMINI_API_KEY repository secret was not available to the workflow.",
        })
        return
    ...
    try:
        ...
        passes = sum(int(row["pass"]) for row in results)
        write_result({
            "summary": {
                **base_summary,
                "status": "API_OK",
        ...
        write_result({
            "summary": {**base_summary, "status": "API_ERROR", "passes": 0, "pass_rate": 0.0},
            "results": [],
            "diagnostic": repr(exc),
        })
